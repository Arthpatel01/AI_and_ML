...
def cartoonify(ImagePath):
    ...
    if originaimage is None:
        ...
        sys.exit()
    ReSized1 = cv2.resize(originaimage, (652, 1311))
    
    #img to grayscale
    grayScaleImage = cv2.cvtColor(originaimage, cv2.COLOR_BGR2GRAY)
    ReSized2 = cv2.resize(grayScaleImage, (652, 1311))
    
    #smoothing image
    smoothGrayScale = cv2.medianBlur(grayScaleImage, 5)
    ReSized3 = cv2.resize(smoothGrayScale, (652, 1311))
    
    
    #retriving edge
    getEdge = cv2.adaptiveThreshold(smoothGrayScale, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 9, 9)
    ReSized4 = cv2.resize(getEdge, (652, 1311))
    
    
    #mask
    colorImage = cv2.bilateralFilter(originaimage, 9, 300, 300)
    ReSized5 = cv2.resize(colorImage, (652, 1311))
    
    #combine for cartoon
    cartoonImage = cv2.bitwise_and(colorImage, colorImage, mask=getEdge)
    ReSized6 = cv2.resize(cartoonImage, (652, 1311))
